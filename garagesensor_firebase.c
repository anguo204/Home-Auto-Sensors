...
void loop() {
    ...
    if( digitalRead(DoorGpioPin)==HIGH && doorState==opened){ // Print button pressed message.
        ...
        Firebase.setBool("sensors/garageopen", false);
        if (Firebase.failed()) {
            Serial.print("setting /false failed:");
            Serial.println(Firebase.error());
            return;
        }
        Serial.println("sensors/garageopen = false");
    }
    yield();
    if( digitalRead(DoorGpioPin)==LOW && doorState==closed){
        ...
        //update boolean variable in FIREBASE
        Firebase.setBool("sensors/garageopen", true);
        if (Firebase.failed()) {
            Serial.print("setting /truth failed:");
            Serial.println(Firebase.error());
            return;
        }
        Serial.println("sensors/garageopen = true");
    }
